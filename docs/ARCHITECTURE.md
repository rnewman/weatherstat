...
**Stage 2 — Effector gains, solar, and environment effects (ridge regression):** With tau_base fitted, computes Newton residuals at every timestep (`dT/dt_observed - dT/dt_newton`). These residuals are explained by a linear regression on: lagged effector activity (coarse time bins capturing delay), a solar elevation feature (`sin⁺(elevation) × weather_fraction` — one continuous feature replacing the prior 11 per-hour indicators), weather control features (ΔT², wind×ΔT, dT_outdoor/dt), per-environment-entry `state × (T_out - T)` features (cooling/warming rate when active), and pairwise interaction terms (e.g., cross-breeze effects between two open windows). One regression per sensor.
