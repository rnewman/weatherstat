...
5. **Learned over configured.** Sysid discovers physics (tau, gains, environment effects) from data. Configuration declares what exists, not how things interact. Command/state pairs: every effector has intent (command) and reality (measured state). Sysid trains on state; control sets commands.
...
- **Environment** — observable factors that affect physics but aren't system-controlled (windows, doors, shades, vents, space heaters). Each has a `kind` (driving display verbs), `default_state`/`active_state`, and an optional `advisory: true` flag for inclusion in the trajectory sweep. Their effect on sensor dynamics is learned by sysid (per-entry tau coefficients in TauModel)
...
- `environment` — observable factors affecting physics: windows, doors, shades, vents, space heaters. Each entry declares `kind`, `entity_id`, `column`, `default_state`/`active_state`, and optional `advisory: true`. Advisory entries are included in the trajectory sweep for proactive recommendations. Replaces the former separate `windows` and `advisory_effectors` sections
...
- Extracts values using config-driven column definitions (temperature attributes, HVAC actions/modes/targets, environment states, weather conditions).
...
**Output:** SQLite database with 5-minute resolution. The `readings` table stores `(timestamp, name, value)` triples (count depends on configured sensors/effectors/forecasts). The Python reader pivots this to a wide DataFrame at load time, applying types from config.
...
Where:
- `tau` is the effective envelope time constant, computed from `TauModel`: `1/tau_eff = 1/tau_base + Σ β_e × active_e + Σ β_{ee'} × active_e × active_e'` (where `e` ranges over environment entries — windows, doors, vents, etc.)
...
**Stage 1 — Tau fitting (scipy `curve_fit`):** For each temperature sensor, selects all nighttime (10pm–6am) periods where all HVAC effectors are off AND all environment factors are in a stable (constant) state throughout the segment — open windows are fine as long as they stay open. Fits Newton cooling (`T(t) = T_out + (T_0 - T_out) * exp(-t/tau)`) via nonlinear least squares on each contiguous segment. Multiple segments → weighted median → `tau_base` (sealed envelope time constant).

**Stage 2 — Effector gains, solar, and environment effects (ridge regression):** With tau_base fitted, computes Newton residuals at every timestep (`dT/dt_observed - dT/dt_newton`). These residuals are explained by a linear regression on: lagged effector activity (coarse time bins capturing delay), a solar elevation feature (`sin⁺(elevation) × weather_fraction` — one continuous feature replacing the prior 11 per-hour indicators), weather control features (ΔT², wind×ΔT, dT_outdoor/dt), time-of-day Fourier features (sin/cos at 24h and 12h periods — absorb diurnal internal-heat patterns from occupancy, cooking, and stored wall heat that correlate with HVAC activity but aren't caused by it), per-environment-entry `state × (T_out - T)` features (cooling/warming rate when active), and pairwise interaction terms (e.g., cross-breeze effects between two open windows). One regression per sensor.

The regression uses selectively standardized ridge (L2 penalty λ = 0.01×n). Solar and environment features are pre-scaled by their standard deviation so the penalty falls proportionally; effector features are left in raw scale for full regularization against confounded gains. T-statistics flag negligible gains (|gain| < 0.05°F/hr AND |t-stat| < 2.0).
...
- **Environment coupling coefficients**: per-entry `β_e` (additional cooling/warming rate when active) and pairwise interaction terms `β_{ee'}`. The simulator computes effective tau as `1 / (1/tau_base + Σ β_e × active_e + Σ β_{ee'} × active_e × active_e')`.
...
2. **Hard rails:** Steep additional penalty (default 3×, configurable via `defaults.hard_rail_multiplier`) for exceeding `min`/`max` bounds.
...
Active environment states (e.g., open windows) widen the comfort band to avoid fighting ventilation.
...
- Hold times: 3-minute minimum between setpoint changes.
- Mode hold window: per-device configurable hours (e.g., 10pm–7am via `mode_hold_window`) during which mini-split mode changes are forbidden — only silent target temperature adjustments are allowed.
...
### 6. Environment Advisory (`src/weatherstat/advisory.py`)

Persistent, energy-aware recommendations for environment state changes. Uses the physics simulator to evaluate whether toggling environment entries (windows, doors, shades, vents, space heaters) would improve comfort and/or save energy, given the committed electronic plan.

**Two-tier evaluation:**
1. **Quick check:** Simulate winning scenario with entry toggled → comfort delta.
2. **Re-sweep** (if promising): Full scenario sweep with toggled entry to find the best HVAC plan. Captures "open window + turn off mini split" energy savings.
...
**Lifecycle management:** Opportunities persist across control cycles. New ones are added, still-valid ones kept, expired ones dismissed (including HA persistent notifications). Per-entry notification IDs prevent stacking.

**Advisory entries** (environment entries with `advisory: true`) are user-operated devices that the system observes, models in physics, sweeps in trajectory search, but only advises on. The trajectory sweep crosses HVAC scenarios with advisory options (hold current state, return to default at various timings, proactive activation). Three planning layers are extracted from a single sweep: *reasonable* (user cooperates at optimal time — drives HVAC decisions), *worst-case* (user does nothing — backup breach detection), and *proactive* (activate default-state devices — "open window for 2h"). When advisory combinatorics exceed the hard cap (many active entries × many options × HVAC scenarios), a two-stage sweep is used: score HVAC-only first (cheap scalar tau), select top-K, then cross with advisory product (full per-step tau dynamics).

**Key design constraint:** The electronic plan does NOT change based on opportunities. The controller commits to the best electronic trajectory given current environment states, modulated by the reasonable advisory plan (cooperative assumption). If the human acts (e.g., opens a window), the next control cycle re-evaluates and naturally adjusts.

**Dispatch:**
- Per-entry cooldown timers prevent notification fatigue.
...
- Resolved comfort bounds per sensor (min, max, preferred_lo, preferred_hi, cold/hot penalties) — the actual targets after profile offsets, MRT correction, and environment adjustments.
...
- One snapshot per 5-minute interval (reading count depends on config).
- Names include: temperature sensors, HVAC states (action, mode, target per effector), environment states, weather, humidity sensors, forecast snapshots.
...
1. `evaluate_environment_opportunities(state, winning_scenario, ...)` — runs `predict()` per environment entry toggle, re-sweeps promising candidates for energy savings, returns opportunity list.
