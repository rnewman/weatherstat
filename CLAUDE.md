...
30. **Superposition-based sweep** (done) — Marginal decomposition exploits linearity of the Euler forcing terms: `T(A=a, B=b, ...) = T_base + δT_A(a) + δT_B(b) + ...`. Instead of simulating all ~500K effector combinations, simulate ~170 marginals (all-off baseline + one per effector option) and reconstruct combination temperatures via array addition. Three-phase sweep: (1) marginal simulation, (2) approximate combination scoring via meshgrid + vectorized comfort/energy cost, (3) exact Euler refinement of top-K (~100 candidates). Dependent effectors (blowers gated by thermostats) form compound groups — their activity is multiplicative, not additive, so parent+dependents are simulated as a unit. Regulating effectors break linearity slightly (proportional activity depends on T); approximate scoring is within ~1°F, and exact refinement corrects. Advisory effectors (windows, shades) modulate τ multiplicatively, fully breaking superposition — they continue full Euler simulation in Stage 2. `_enumerate_options()` extracted from `generate_trajectory_scenarios`. New: `EffectorOptionGroup`, `MarginalResult`, `_build_option_groups()`, `_compute_marginals()`, `_score_combinations()`, `_materialize_scenarios()`. Cold-sensor override operates on approximate cost arrays, adds constrained-best to top-K for exact verification. Expected 5–9× speedup (11.5s → ~1.2–2.2s).

31. **Gain deconfounding & Fourier time-of-day features** (done) — Investigation into whether cross-room sysid gain `mini_split_living_room → thermostat_upstairs_temp` (+0.608°F/hr) was inflated by time-of-day confounding. Built experiment harness (`scripts/experiment_sysid.py`) for systematic A/B testing of sysid variants against a frozen baseline. Three experiments: (1) time-of-day Fourier features (sin/cos at 24h and 12h periods) absorb diurnal internal-heat patterns — universal R² improvement (+5.2%), minor cross-room gain reduction (-3%), landed; (2) transition-weighted regression (forward-only Gaussian kernel, σ sweep 10–120m) — all gains *increased*, confirming they're causal, not confounded; diagnostic value only, not landed; (3) conditional subsample validation — not needed, transition weighting already answered the causality question. Conclusion: the gains are real; the original over-cooling problem was a comfort scoring/weighting issue, fixed by adjusting `cold_penalty` and widening nighttime preferred ranges. See `docs/plans/PLAN-31-gain-deconfounding.md`.
